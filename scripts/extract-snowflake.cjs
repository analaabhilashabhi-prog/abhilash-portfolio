...
for (let i = lines.length - 1; i >= 0; i--) {
  try {
    ...
    if (parsed.type === 'USER_INPUT' && parsed.content.includes('Snowflake Logo Halftone')) {
      ...
      const paddingFactor = 0.15; // same 15% padding as other logos
      ...
      const outPath = path.join(__dirname, '..', 'src', 'assets', 'logos', 'logo-dotted-3.svg');
