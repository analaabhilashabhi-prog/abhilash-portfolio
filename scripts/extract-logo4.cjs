...
for (let i = lines.length - 1; i >= 0; i--) {
  try {
    ...
    if (parsed.type === 'USER_INPUT' && parsed.content.includes('cy="143.0" r="1.46"')) {
      ...
      const outPath = path.join(__dirname, '..', 'src', 'assets', 'logos', 'logo-dotted-4.svg');
